...
class Currency{
	...
	async setFormat(format)	{
		const CurrencyFormat = (await (import("./currency_format.js"))).MasterModel;
		if((format != null) && (!(format instanceof CurrencyFormat)))	{
			throw new SDKException(Constants.DATA_TYPE_ERROR, "KEY: format EXPECTED TYPE: CurrencyFormat", null, null);
		}
		this.format = format;
