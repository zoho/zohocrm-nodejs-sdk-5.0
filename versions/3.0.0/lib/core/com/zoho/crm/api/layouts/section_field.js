...
class SectionField extends Fields{
	 ...
	 */
	setLength(length)	{
		if((length != null) && (!(length instanceof Choice)))	{
			throw new SDKException(Constants.DATA_TYPE_ERROR, "KEY: length EXPECTED TYPE: Choice", null, null);
		}
		this.length = length;
	 ...
	 * @returns {SectionSubformField} An instance of SectionSubformField
	 */
	getSubform()	{
		return this.subform;
	...
	async setSubform(subform)	{
		const SectionSubformField = (await (import("./section_subform_field.js"))).MasterModel;
		if((subform != null) && (!(subform instanceof SectionSubformField)))	{
			throw new SDKException(Constants.DATA_TYPE_ERROR, "KEY: subform EXPECTED TYPE: SectionSubformField", null, null);
		}
